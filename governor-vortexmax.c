...
/*
 * VortexMax CPU Governor v1.0 (Base Version)
...
/* VortexMax Heuristic Parameters */
...
struct vortexmax_cpu_info {
    ...
    unsigned int max_hold_counter; /* Max Hold State */
};
...
{
    struct vortexmax_policy_info *vpinfo = container_of(work, struct vortexmax_policy_info, work.work);
...
/* ========================================================================
 * GKI 5.10 HYBRID API STRUCT
 * ======================================================================== */
static int vortexmax_init(struct cpufreq_policy *policy)
{
    struct vortexmax_policy_info *vpinfo = kzalloc(sizeof(*vpinfo), GFP_KERNEL);
    if (!vpinfo)
        return -ENOMEM;
...
static int vortexmax_start(struct cpufreq_policy *policy)
{
    ...
    for_each_cpu(cpu, policy->cpus) {
        struct vortexmax_cpu_info *info = &per_cpu(vortexmax_info, cpu);
    ...
    schedule_delayed_work_on(policy->cpu, &((struct vortexmax_policy_info *)policy->governor_data)->work, msecs_to_jiffies(20));
    return 0;
}

static void vortexmax_stop(struct cpufreq_policy *policy)
{
    cancel_delayed_work_sync(&((struct vortexmax_policy_info *)policy->governor_data)->work);
}
...
{
    return cpufreq_register_governor(&vortexmax_gov);
}
...
MODULE_DESCRIPTION("VortexMax v1.0 - Base Version");
MODULE_LICENSE("GPL");
